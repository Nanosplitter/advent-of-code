...
with open(input_file) as f:
    ...
    bytes_limit = int(instructions[1])

    board = Board(size, size)
    
    byte_positions = [tuple(map(int, position.split(","))) for position in instructions[2:]]
    
    for row in range(size):
        for col in range(size):
            symbol = "."
            node = Node((row, col), symbol)
            board.set_node(node)
    
        
    board.set_start_position((0, 0))
    board.set_end_position((size - 1, size - 1))
    
    for position in byte_positions:
        node = board.nodes[position[::-1]]
        node.symbol = "#"
                
        min_cost, nodes_in_best_paths = find_shortest_paths(board)
        print(f"Position: {position}, Min Cost: {min_cost}, Number of nodes in best paths: {len(nodes_in_best_paths)}")
        
        if min_cost == inf:
            print(f"Position {position} makes exit unreachable")
            break
    
    for row in range(board.height):
        for col in range(board.width):
            if (col, row) in nodes_in_best_paths:
